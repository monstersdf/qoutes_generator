...
let colr = [ 'red', 'blue', 'green', 'orange', 'purple', 'yellow'];
let centersdiv = document.getElementById("first");

let bodyqoutes = document.getElementById("qoutes");
console.log(bodyqoutes);
let authors = document.getElementById("authorname");
console.log(authors.innerHTML);

function generateRandomQoutes() {
  const random = qoutesarr[Math.floor(Math.random() * qoutesarr.length)];
  bodyqoutes.innerHTML = random["quote"];
  authors.innerHTML = random["author"];
  console.log(random);
//   centersdiv.style.backgroundColor = colr;
 let vfx = parseInt((Math.random()*colr.length)+1);
  centersdiv.style.background = `${colr[vfx]}`
}
